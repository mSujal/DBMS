...
DB_CONFIG = {
    "host": "localhost",
    # need to add own sql server username and password creatign .evn dir 
    "user": user,
    "password": password,
    "database": "Hotel_Management" # database name as created locally :w
}
...
def create_role():
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Roles (RoleID, Role_Name, Base_Salary) VALUES (%s, %s, %s)",
...
def update_role(role_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Roles SET Role_Name=%s, Base_Salary=%s WHERE RoleID=%s",
...
def create_department():
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Department (DeptID, Dept_Name, ManagerId) VALUES (%s, %s, %s)",
...
def update_department(dept_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Department SET Dept_Name=%s, ManagerId=%s WHERE DeptID=%s",
...
def create_room():
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Room (RoomID, Room_Type, Floor, Base_Price, Room_Status) VALUES (%s, %s, %s, %s, %s)",
...
def update_room(room_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Room SET Room_Type=%s, Floor=%s, Base_Price=%s, Room_Status=%s WHERE RoomID=%s",
...
def update_booking_status(booking_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Booking SET Booking_Status=%s WHERE BookingID=%s",
...
def remove_room_from_booking(booking_id, room_id):
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM Booking_Room WHERE BookingID=%s AND RoomID=%s", (booking_id, room_id))
        conn.commit()
...
def update_payment_status(payment_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Payment SET Payment_Status=%s WHERE PaymentID=%s",
...
def update_service_order_status(order_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Service_Order SET Status=%s WHERE OrderID=%s", (data["Status"], order_id))
        conn.commit()
...
def update_maintenance_status(request_id):
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE Maintenance_Request SET Status=%s WHERE RequestID=%s",
...
if __name__ == "__main__":
    app.run(debug=True, port=5000) # or whatever port used